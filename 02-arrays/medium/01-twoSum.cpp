...
// target. Otherwise, we will return {-1, -1}. Note : You are not allowed to use the same element
// twice.Example : If the target is equal to 6 and num[1] = 3, then nums[1] +
// nums[1] = target is not a solution. first variant
inline bool twoSum1_brute(vector<int> &arr, int k) {
    int n = arr.size();
    for (int i = 0; i < n; i++) {
        for (int j = i + 1; j < n; j++) {
            if (arr[i] + arr[j] == k)
...
// using hash
inline bool twoSum_hash(vector<int> &arr, int target) {
    int n = arr.size();
