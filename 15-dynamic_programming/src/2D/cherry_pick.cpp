...
int changes[3] = {-1, 0, 1};
using DP = cube;
int cherry_pick_recursion(int row, int a_col, int b_col, Vecvec& grid)
{
...
int cherry_pick_memo(int row, int a_col, int b_col, Vecvec& grid, DP& dp)
{
	if (a_col < 0 || a_col >= grid[row].size() || b_col < 0 || b_col >= grid[row].size())
		return INT_MIN;
	if (row >= grid.size() - 1) {
...
int CherryPicking::recursion(Vecvec& grid)
{
	return cherry_pick_recursion(0, 0, grid[0].size() - 1, grid);
}

int CherryPicking::memoization(Vecvec& grid)
{
...
{
	Vecvec vec = {{2, 3, 1, 2}, {3, 4, 2, 2}, {5, 6, 3, 5}};
	// Vecvec vec = {{4, 1, 2}, {7, 3, 5}};
	int res = 0;
